...
import AsyncCustomMenuList from 'components/common/Select/AsyncCustomMenuList';
...
class CustomSelect<OptionValue> extends React.Component<Props<OptionValue>, State> {
  static defaultProps = {
  ...
  createCustomFilter = (filterOption) => {
    const { ignoreAccents } = this.props;

    return createFilter({ ignoreAccents, ignoreCase: false, stringify: (option: { label: unknown }) => String(option.label) });
  };
...
export default withTheme(CustomSelect) as unknown as React.ComponentType<Omit<Props<any>, 'theme'>>;
